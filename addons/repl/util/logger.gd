...
# Includes partial version of katuiche-colorful-console in logger-colors.gd
#   https://github.com/CassianoBelniak/katuiche-colorful-console
#
# TODO: Use colorful-console as a dependency if possible
...
class DebugPrintBase:
	...
	# `write` becuase `log` is not a legal literal and there's no callable instances
